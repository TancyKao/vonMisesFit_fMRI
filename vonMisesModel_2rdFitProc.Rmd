...
VD <- function(X,Y){
  if(length(X)==1) 
  {k=1}
  else if (length(X)>=2 & Y[2]>0.01)
  {k=1}
  else if (length(X)==2 & Y[2]<=0.01)
  {k=2}
  else if (length(X)==3 & Y[2]<=0.01 & Y[3]>0.01)
  {k=2}
  else #(length(X)==3 & Y[2]<=0.001 & Y[3]<=0.001)
  {k=3}
  return(k)
}
...
FreqFtest_Table[2,1] <- 134
FreqFtest_Table[2,3] <- 4
...
BestMd_Ftest_prefOrient <- BestMd_Ftest_prefOrient[!(BestMd_Ftest_prefOrient$SubjNew=='S04'& BestMd_Ftest_prefOrient$FitVM==3 & BestMd_Ftest_prefOrient$Voxel>500),]
BestMd_Ftest_prefOrient <- BestMd_Ftest_prefOrient[!(BestMd_Ftest_prefOrient$SubjNew=='S04'& BestMd_Ftest_prefOrient$FitVM==5 & BestMd_Ftest_prefOrient$Voxel>500),]
...
Best_Peak <- Best_Peak[!(Best_Peak$SubjNew=='S04'& Best_Peak$FitVM_pref==3 & Best_Peak$Voxel>200),]
Best_Peak <- Best_Peak[!(Best_Peak$SubjNew=='S04'& Best_Peak$FitVM_pref==4 & Best_Peak$Voxel>200),]
...
dev.new()
ggplot(subset(Corr_prefOrientAvgDat, SubjNew=='S04'), aes(x=factor(PrefPair2), y=Corr)) +
  theme_few() +
